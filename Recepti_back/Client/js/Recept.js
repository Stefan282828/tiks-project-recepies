  ...
  try {
    await safeFetch(`${apiBaseRecept}/Dodaj`, {
      ...
      body: JSON.stringify({
        ...
        opis,
        vremePripreme: vreme,
        uputstvoPripreme: uputstvo,
        podKategorijaId: podId
      })
    });
    form.reset();
    loadRecepti();
...
window.addEventListener('load', async () => {
  // 🔹 Ispravan link za povratak
  const back = document.querySelector('.header-link');
  if (back) {
    back.setAttribute(
      'href',
      `Podkategorija.html?categoryId=${categoryId}&categoryName=${encodeURIComponent(categoryName)}`
    );
  }
  setHeader();
