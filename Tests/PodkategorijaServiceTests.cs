...
public class PodkategorijaServiceTests
{
    ...
    public async Task CreatePodkategorija_WithKategorijaId_ShouldPersistAndLink()
    {
        ...
        var kat = await katService.CreateKategorijaAsync(new FoodExplorer.Models.Dto.KategorijaCreateDto { Naziv = "Italijanska" });

        var pod = await podService.CreatePodkategorijaAsync(new FoodExplorer.Models.Dto.PodkategorijaRequestDto { Naziv = "Pasta", KategorijaId = kat.Id });
    ...
    public async Task GetByKategorijaId_ShouldReturnOnlyMatching()
    {
        ...
        var katService = new KategorijaService(ctx);
        var podService = new PodkategorijaService(ctx);
        var kat1 = await katService.CreateKategorijaAsync(new FoodExplorer.Models.Dto.KategorijaCreateDto { Naziv = "Italijanska" });
        var kat2 = await katService.CreateKategorijaAsync(new FoodExplorer.Models.Dto.KategorijaCreateDto { Naziv = "Meksička" });

        await podService.CreatePodkategorijaAsync(new FoodExplorer.Models.Dto.PodkategorijaRequestDto { Naziv = "Pasta", KategorijaId = kat1.Id });
        await podService.CreatePodkategorijaAsync(new FoodExplorer.Models.Dto.PodkategorijaRequestDto { Naziv = "Tacos", KategorijaId = kat2.Id });
    ...
    public async Task UpdatePodkategorija_ShouldChangeName()
    {
        ...
        var podService = new PodkategorijaService(ctx);
        var kat = await katService.CreateKategorijaAsync(new FoodExplorer.Models.Dto.KategorijaCreateDto { Naziv = "Italijanska" });
        var pod = await podService.CreatePodkategorijaAsync(new FoodExplorer.Models.Dto.PodkategorijaRequestDto { Naziv = "Staro", KategorijaId = kat.Id });

        var updated = await podService.UpdatePodkategorijaAsync(pod.Id, new FoodExplorer.Models.Dto.PodkategorijaRequestDto { Naziv = "Novo", KategorijaId = kat.Id });
